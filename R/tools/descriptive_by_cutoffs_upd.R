kCCQ_TABS <- c("watch_dm_11_high_risk_cu","Overall_Score_9100cu",
               "Overall_Score_90cu", "Overall_Score_80cu", "Overall_Score_75cu",
               "Clinical_Score_9100cu","Clinical_Score_90cu", "Clinical_Score_80cu","Clinical_Score_75cu",
               "Symptom_Score_9100cu","Symptom_Score_90cu", "Symptom_Score_80cu", "Symptom_Score_75cu",
               "Physical_Score_9100cu", "Physical_Score_90cu", "Physical_Score_80cu", "Physical_Score_75cu","watch_dm_11_high_risk_sob_cu",
               "watch_dm_11_high_risk_kccq_cu"
               )
...
output_diag <- function(data, kccq_in, echo_in) {
    ...
    additional_eval <- data.frame(
        statistic = c("pre_test_odds", "post_test_odds", "post_test_probability"),
        est = c(NA, NA, NA),
        lower = c(NA, NA,NA),
        upper = c(NA, NA, NA),
        performance = c(
            performance$est[performance$statistic == "tp"] / (1 - performance$est[performance$statistic == "tp"]),
            (performance$est[performance$statistic == "tp"] / (1 - performance$est[performance$statistic == "tp"])) * performance$est[performance$statistic == "lr.pos"],
            ((performance$est[performance$statistic == "tp"] / ((1 - performance$est[performance$statistic == "tp"]) * performance$est[performance$statistic == "lr.pos"])))/
                ((performance$est[performance$statistic == "tp"] / ((1 - performance$est[performance$statistic == "tp"]) * performance$est[performance$statistic == "lr.pos"]))+1)
        ))
   ...
   # Return summarised results
    resutls <- tibble(
        ...
        pre_test_odds =  additional_eval$performance[additional_eval$statistic=="pre_test_odds"],
        post_test_odds =  additional_eval$performance[additional_eval$statistic=="post_test_odds"],
        post_test_probability =  additional_eval$performance[additional_eval$statistic=="post_test_probability"],
        youden = performance$performance[performance$statistic=="youden"]
        )
...
output_diag_sob <- function(data, echo_in) {
    ...
    # Return summarised results
    resutls <- tibble(
        kccq = "Any symptoms of shotness of breath or swealling",
        echo = echo_in,
        sensitivity = performance$performance[performance$statistic=="se"],
        specificity = performance$performance[performance$statistic=="sp"],
        tpp = performance$performance[performance$statistic=="pv.pos"],
...
all_outputs_sob <- result_sob %>%
    ...
    list_rbind() %>%
    mutate(kccq = "Any symptoms of shotness of breath or swealling")
...
kbl(results_table_knitr[, -1])  %>%
    ...
    group_rows("Clinical Score 100 cutoff", 17, 20)  %>%

    group_rows("Overall Score 75 cutoff", 21, 24)  %>%
    group_rows("Overall Score 80 cutoff", 25, 28)  %>%
    group_rows("Overall Score 90 cutoff", 29, 32)  %>%
    group_rows("Overall Score 100 cutoff", 33, 36)  %>%

    group_rows("Physical Score 75 cutoff", 37, 40)  %>%
    group_rows("Physical Score 80 cutoff", 41, 44) %>%
    group_rows("Physical Score 90 cutoff", 45, 48)  %>%
    group_rows("Physical Score 100 cutoff", 49, 52)  %>%

    group_rows("Symptom Score 75 cutoff", 53, 56)  %>%
    group_rows("Symptom Score 80 cutoff", 57, 60) %>%
    group_rows("Symptom Score 90 cutoff", 61, 64) %>%
    group_rows("Symptom Score 100 cutoff", 65, 68) %>%
    group_rows("WATCH-DM high risk (score > 11)", 69, 72) %>%
    group_rows("Symptoms + WATCH-DM high risk (score > 11)", 73, 76) %>%
    group_rows("KCCQ-symptoms + WATCH-DM high risk (score > 11)", 77, 80)



# Define the desired order
desired_order <- c("Any symptoms of shotness of breath or swealling", "Clinical_Score_9100cu", "Clinical_Score_90cu",
                   "Clinical_Score_80cu", "Clinical_Score_75cu", "watch_dm_11_high_risk_cu", "watch_dm_11_high_risk_sob_cu",
                   "watch_dm_11_high_risk_kccq_cu")

# Apply the filter and sort the `kccq` column by the desired order
results_table_small <- results_table_final %>%
    filter(kccq %in% desired_order) %>%
    mutate(kccq = factor(kccq, levels = desired_order)) %>%
    arrange(kccq)

colnames(results_table_small) <- c(
    "KCCQ",
    "category",
    "Normal",
    "Abnormal",
...
kbl(results_table_small[, -1])  %>%
    ...
    add_header_above(c("Symptoms" = 1, "Diastolic dysfunction (E/e’≥ 14)" = 2,
                       ...
                       "Left ventricular hypertrophy (LVMI ≥95 g/m2 (F), ≥115 g/m2 (M))" = 2, "Any subtype of HF" = 2)) %>%
    group_rows("Any symptoms of shotness of breath or swealling", 1,4) %>%  # Group rows 1-2 under "Category A"
    group_rows("Clinical Score 100 cutoff", 5, 8) %>%
    group_rows("Clinical Score 90 cutoff", 9, 12)  %>%
    group_rows("Clinical Score 80 cutoff", 13, 16)  %>%
    group_rows("Clinical Score 75 cutoff", 17, 20)  %>%

    group_rows("WATCH-DM high risk (score > 11)", 21, 24) %>%
    group_rows("Symptoms + WATCH-DM high risk (score > 11)", 25, 28) %>%
    group_rows("KCCQ-symptoms + WATCH-DM high risk (score > 11)", 29, 32)

# Define the desired order
desired_order <- c("Any symptoms of shotness of breath or swealling", "Clinical_Score_90cu", "Clinical_Score_75cu", "watch_dm_11_high_risk_cu", "watch_dm_11_high_risk_sob_cu")
...
kbl(results_table_small[, c(2, (ncol(results_table_small)-1):ncol(results_table_small))])  %>%
    kable_classic() %>%
    add_header_above(c("Symptoms" = 1, "Any subtype of HF" = 2)) %>%
    group_rows("Any symptoms of shotness of breath or swealling", 1,4) %>%  # Group rows 1-2 under "Category A"
    group_rows("Clinical Score 90 cutoff", 5, 8)  %>%
    group_rows("Clinical Score 75 cutoff", 9, 12)  %>%
    group_rows("WATCH-DM high risk (score > 11)", 13, 16)%>%
    group_rows("Symptoms + WATCH-DM high risk (score > 11)", 17, 20)

kbl(results_table_small[, c(2: 10)])  %>%
    kable_classic() %>%
    add_header_above(c("Symptoms" = 1, "Diastolic dysfunction (E/e’≥ 14)" = 2,
                       "GLS impaired (GLS ≥ −16%)" = 2,
                       "Left atrium enlargement (Left atrial volume index >34 ml/m2)" = 2,
