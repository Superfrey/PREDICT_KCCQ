...
study_data_male <- study_data_fct %>%
    filter(gender == 1)
...
result <- expand_grid(kccq = kCCQ_TABS, echo = hf_diagnosis) %>%
    ...
    mutate(distribution = list(
        ...
        as.data.frame(table(study_data_male[[kccq]], study_data_male[[echo]])) %>%
...
result_sob <- expand_grid(echo = hf_diagnosis) %>%
    ...
    mutate(distribution = list(
        # Generate the table and create a data frame with counts and percentages
        as.data.frame(table(study_data_male$hf_symptoms, study_data_male[[echo]])) %>%
            mutate(percentage = Freq / sum(Freq) * 100)
    )) %>%
    unnest(distribution) %>%
    mutate(distribution = paste0(Freq ," (", round(percentage,1),")"))
...
results_table_knitr_men <- results_table_final
colnames(results_table_knitr_men) <- c(
    "KCCQ",
    "category",
    "Normal",
    "Abnormal",
...
study_data_female <- study_data_fct %>%
    filter(gender == 2)
...
result <- expand_grid(kccq = kCCQ_TABS, echo = hf_diagnosis) %>%
    ...
    mutate(distribution = list(
        # Generate the table and create a data frame with counts and percentages
        as.data.frame(table(study_data_female[[kccq]], study_data_female[[echo]])) %>%
            mutate(percentage = Freq / sum(Freq) * 100)
    )) %>%
    unnest(distribution) %>%
    mutate(distribution = paste0(Freq ," (", round(percentage,1),")"))
...
result_sob <- expand_grid(echo = hf_diagnosis) %>%
    ...
    mutate(distribution = list(
        # Generate the table and create a data frame with counts and percentages
        as.data.frame(table(study_data_female$hf_symptoms, study_data_female[[echo]])) %>%
            mutate(percentage = Freq / sum(Freq) * 100)
    )) %>%
    unnest(distribution) %>%
    mutate(distribution = paste0(Freq ," (", round(percentage,1),")"))
...
results_table_final <- bind_rows(results_table_sob, results_table)
results_table_knitr_women <- results_table_final
colnames(results_table_knitr_women) <- c(
    "KCCQ",
    "category",
    "Normal",
    "Abnormal",
...
###############################################################################
# Print the result
kbl(results_table_knitr_women[, -1])  %>%
    kable_classic() %>%
    add_header_above(c("Symptoms" = 1, "Diastolic dysfunction (E/e’≥ 14)" = 2,
                       "GLS impaired (GLS ≥ −16%)" = 2,
                       "Left atrium enlargement (Left atrial volume index >34 ml/m2)" = 2,
...
study_data_below72 <- study_data_fct %>%
    filter(age < 72)
...
result <- expand_grid(kccq = kCCQ_TABS, echo = hf_diagnosis) %>%
    ...
    mutate(distribution = list(
        # Generate the table and create a data frame with counts and percentages
        as.data.frame(table(study_data_below72[[kccq]], study_data_below72[[echo]])) %>%
            mutate(percentage = Freq / sum(Freq) * 100)
    )) %>%
    unnest(distribution) %>%
    mutate(distribution = paste0(Freq ," (", round(percentage,1),")"))
...
result_sob <- expand_grid(echo = hf_diagnosis) %>%
    ...
    mutate(distribution = list(
        # Generate the table and create a data frame with counts and percentages
        as.data.frame(table(study_data_below72$hf_symptoms, study_data_below72[[echo]])) %>%
            mutate(percentage = Freq / sum(Freq) * 100)
    )) %>%
    unnest(distribution) %>%
    mutate(distribution = paste0(Freq ," (", round(percentage,1),")"))
...
results_table_final <- bind_rows(results_table_sob, results_table)
results_table_knitr_below72 <- results_table_final
colnames(results_table_knitr_below72) <- c(
    "KCCQ",
    "category",
    "Normal",
    "Abnormal",
...
###############################################################################
# Print the result
kbl(results_table_knitr_below72[, -1])  %>%
    kable_classic() %>%
    add_header_above(c("Symptoms" = 1, "Diastolic dysfunction (E/e’≥ 14)" = 2,
                       "GLS impaired (GLS ≥ −16%)" = 2,
                       "Left atrium enlargement (Left atrial volume index >34 ml/m2)" = 2,
...
study_data_above72 <- study_data_fct %>%
    filter(age >= 72)
...
result <- expand_grid(kccq = kCCQ_TABS, echo = hf_diagnosis) %>%
    ...
    mutate(distribution = list(
        # Generate the table and create a data frame with counts and percentages
        as.data.frame(table(study_data_above72[[kccq]], study_data_above72[[echo]])) %>%
            mutate(percentage = Freq / sum(Freq) * 100)
    )) %>%
    unnest(distribution) %>%
    mutate(distribution = paste0(Freq ," (", round(percentage,1),")"))
...
result_sob <- expand_grid(echo = hf_diagnosis) %>%
    ...
    mutate(distribution = list(
        # Generate the table and create a data frame with counts and percentages
        as.data.frame(table(study_data_above72$hf_symptoms, study_data_above72[[echo]])) %>%
            mutate(percentage = Freq / sum(Freq) * 100)
    )) %>%
    unnest(distribution) %>%
    mutate(distribution = paste0(Freq ," (", round(percentage,1),")"))
...
results_table_final <- bind_rows(results_table_sob, results_table)
results_table_knitr_above72 <- results_table_final
colnames(results_table_knitr_above72) <- c(
    "KCCQ",
    "category",
    "Normal",
    "Abnormal",
...
###############################################################################
# Print the result
kbl(results_table_knitr_above72[, -1])  %>%
    kable_classic() %>%
    add_header_above(c("Symptoms" = 1, "Diastolic dysfunction (E/e’≥ 14)" = 2,
                       "GLS impaired (GLS ≥ −16%)" = 2,
                       "Left atrium enlargement (Left atrial volume index >34 ml/m2)" = 2,
