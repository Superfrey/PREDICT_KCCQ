...
Too few cases in the different cutoff for HF diagnogsis based on ECHO
...
echo_data %>%
    ...
    flowchart::fc_filter(!is.na(hf_symptoms),
                         label = 'No respond on HF symptoms', show_exc = T) %>%
    flowchart::fc_filter(hf_hx == 0,
                         label = 'Without dianogsed heart failure', show_exc = T) %>%
    flowchart::fc_filter(hf_symptoms==0 | (hf_symptoms==1 & kccq_complete == 2),
